...
CrewOps `v0.4.0` is the M5 commercial field-service showcase. One deterministic `std-web-ui` reducer and one seed-backed backend serve technician, dispatcher, supervisor, and manager workflows across web, desktop, iOS, and Android, now with pricing, invoicing, payments, receivables, exports, and finance rollups in the same app.
...
- Release line: `v0.4.0`
- Frontend dependency baseline: [`frontend/x07.json`](frontend/x07.json) uses `std-web-ui@0.2.2`
- Roles: `technician`, `dispatcher`, `supervisor`, `manager`
- Primary routes: `today`, `dispatch`, `review`, `manager`, `finance`, `pricing`, `invoices`, `activity`, `customers`, `receivables`, `exports`, `sites`, `assets`, `settings`
...
- Commercial seed families now include price books and pricing policies, invoices and artifacts, payments and allocations, customer statements, receivable summaries, export jobs, finance rollups, and profitability snapshots
...
[`scripts/ci/check_all.sh`](scripts/ci/check_all.sh) remains the canonical CrewOps gate for lock, tests, replay, build, pack, verify, provenance, SLO, desktop smoke, and device packaging.

## Release Coverage

The `v0.4.0` release bar is the M5 end-to-end loop:

- dispatch, review, correction, activity, and technician offline execution
- pricing config and billing policy views
- invoice generation, detail edit, issue, void, payment posting, and artifact fetch
- customer account and statement drill-down
- receivables, finance rollups, profitability, and export job views
- deterministic sync and conflict handling for pricing revision, invoice lock, payment revision, export status, and finance revision state

## Device Packaging

The checked-in dev device profiles are now versioned at `0.4.0`:
