...
CrewOps `v0.6.0` keeps one repo, one deterministic reducer, and one seed-backed backend. This release extends the operations and finance core with customer portal, tenant administration, inventory, procurement, vendor connectors, and hosted-readiness reporting without splitting the app into separate products.
...
CrewOps keeps one shared state tree rather than one reducer per role. It extends that tree with:
...
- [`frontend/src/state.x07.json`](../frontend/src/state.x07.json)
  - default UI, selected ids for the operational, finance, portal, and enterprise surfaces, and `0.6.0` app metadata
...
- [`frontend/src/routes.x07.json`](../frontend/src/routes.x07.json)
  - route selection for both the legacy operations routes and the enterprise surfaces
...
CrewOps adds dedicated deterministic handlers for:
